var errorAll =  {
	...
	_114: "iOS 推送存储的 deviceToken 无效，如何存储 installation 请阅读↵					消息推送开发指南。",
	...
	_218: "无效的密码，不允许空白密码。",
	_219: "如果在 15 分钟内，同一个用户登录失败的次数大于 6 次，该用户账户即被云端暂时锁定。锁定将在最后一次错误登录的 15 分钟之后由云端自动解除。如果需要立刻解锁，可以先重置密码再登录。",
	_250: "连接的第三方账户没有返回用户唯一标示 id",
	_251: "无效的账户连接，一般是因为 access token 非法引起的。",
	_252: "无效的微信授权信息。",
	_300: "CQL 语法错误。详情参考↵CQL 语法详细指南↵",
	...
	_430: "超过应用的流控限制，即超过每个应用同一时刻最多可使用的工作线程数，或者说同一时刻最多可以同时处理的数据请求。通过 控制台 > 存储 > API 统计 > API 性能 > 总览 可以查看应用产生的请求统计数据，如平均工作线程、平均响应时间等。使用 LeanCloud↵					商用版或企业版 的用户，如有需要，可以联系我们来调整工作线程数。",
	...
	_1006: "渲染短信模板失败，通常是模板语法问题，我们的短信模板仅支持↵					handlerbars 模板语法。",
	...
	_4110: "登录超时，连接后长时间没有完成 session open。通常是登录被拒绝等原因，出现此问题可能是使用方式有误，可以↵					创建工单，由我们技术顾问来给出建议。",
